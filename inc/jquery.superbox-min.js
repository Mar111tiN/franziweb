...
k();if(B=="gallery"){c(C,E[0])}y(function(){var H=false,G;if(D){H=D.split("x")}G=i('<img src="'+C.attr("href")+'" title="'+(C.attr("title")||C.text())+'" /><p class="picinfo"><strong>'+(C.attr("alt")||C.text())+'</strong></p>');
...
B.get(0)._superboxGroupKey=(x[C[0]].length-1);i.superbox.image(B,A,"gallery")},content:function(B,A){var C=f(B.get(0));B.click(function(D){D.preventDefault();
...
e({boxClasses:"content "+A.boxClasses,boxId:A.boxId});u()})})}});function f(A){return A._relSettings.match(/([^\[\]]+)/g)
